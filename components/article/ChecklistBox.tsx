...
export default function ChecklistBox({ id, title, children }: { id?: string; title?: string; children: ReactNode }) {
  return (
    <div id={id} className="not-prose my-8 scroll-mt-20 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-6">
      <p className="mb-3 flex items-center gap-2 text-[12px] font-bold uppercase tracking-wide text-emerald-700">
        <span aria-hidden>✅</span> {title ?? '체크리스트'}
      </p>
      <div
        className="prose prose-sm sm:prose-base max-w-none text-[#1d1d1f] prose-p:my-2
          ...
          [&_li]:before:rounded-full [&_li]:before:bg-emerald-500 [&_li]:before:text-[11px] [&_li]:before:font-bold
